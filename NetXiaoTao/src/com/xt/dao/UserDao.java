...
public class UserDao {
	...
	public User login(User user) {
		return (User) sessionFactory.getCurrentSession()
				...
				.setString(1, user.getPasswd()).uniqueResult();
	}
	...
	public boolean exits(User user) {
		User u = (User) sessionFactory.getCurrentSession()
				...
				.setString(0, user.getNickname()).uniqueResult();
	...
	public List<Bill> findBillByUseridForPage(String userid, int pageSize,
			int page) {
		String hql = "from Bill where userid=? order by createtime desc";
		return sessionFactory.getCurrentSession().createQuery(hql)
